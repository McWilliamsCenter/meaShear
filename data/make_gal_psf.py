...
'''
def sim_test_data(itest=1,gal_type='knots'):
    ngrid       =   64
    nx          =   1
    ny          =   4
    ndata       =   nx*ny
    ...
    while i <ndata:
        ...
        if i%nrot==0:
            # make galaxy
            if gal_type=='knots':
                gal0    =   galsim.RandomKnots(half_light_radius=0.6,\
                        npoints=npoints,flux=1.)
            elif gal_type=='sersic':
                gal0    =   galsim.Sersic(1.2, half_light_radius=0.1,flux=1.)
    ...
    galFname    =   'gal_test_%d.fits' %(itest)
    pyfits.writeto(galFname,galImg.array,overwrite=True)
    psfFname    =   'psf_test_%d.fits' %(itest)
    pyfits.writeto(psfFname,psfImg.array,overwrite=True)
    return

if __name__ == '__main__':
    sim_test_data(itest=1,gal_type='knots')
    sim_test_data(itest=2,gal_type='sersic')
